...
do
        ...
        az deployment group create \
                ...
                --template-uri https://raw.githubusercontent.com/JLLormeau/lab-environment-for-dynatrace-training/master/azuredeploy-ubuntu22.json \
        ...
        az vm run-command invoke -g "$RESOURCE_GROUP" -n "$DOMAIN" --command-id RunShellScript --scripts "sudo apt-get update && sudo apt-get install shellinabox && sudo sed -i 's/4200/443/g' /etc/default/shellinabox && sudo apt-get install -y docker-compose && cd /home/user"$X$i"/.ssh && echo '"$RSAPUB1" "$RSAPUB2" "$RSAPUB3"'>> authorized_keys && cd /home/user"$X$i" && wget https://raw.githubusercontent.com/JLLormeau/lab-environment-for-dynatrace-training/refs/heads/main/letsencrypt.sh && sudo sh /home/user"$X$i"/letsencrypt.sh "$DOMAIN"."$LOCATION".cloudapp.azure.com user"$X$i"";
 	#az vm run-command invoke -g "$RESOURCE_GROUP" -n "$DOMAIN" --command-id RunShellScript --scripts "sudo apt-get update && sudo snap install --classic certbot && sudo apt-get install shellinabox  && sudo sed -i 's/4200/443/g' /etc/default/shellinabox && sudo apt-get install -y docker-compose && cd /home/user"$X$i"/.ssh && echo '"$RSAPUB1" "$RSAPUB2" "$RSAPUB3"'>> authorized_keys && cd /home/user"$X$i" && sudo certbot certonly --standalone -d $domain --key-type rsa --quiet --agree-tos && wget https://raw.githubusercontent.com/JLLormeau/lab-environment-for-dynatrace-training/refs/heads/main/letsencrypt.sh && sudo sh /home/user"$X$i"/letsencrypt.sh "$DOMAIN"."$LOCATION".cloudapp.azure.com user"$X$i"";
